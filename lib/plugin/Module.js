...
class Module extends EventEmitter {
  constructor(options) {
    ...
    this.config = this.root.child(`config/${this.cid}`);

    // Configuration event
    if (options.useBasicConfig !== false) { // undefined = enabled (for now)
      this.config.on('value', (d) => {
  ...
  response(msg, options = {}) {
    ...
    return new Promise((res, rej) => {
      ...
      const message = {
        ...
        text: msg.command,
        channel: msg.arg,
        data: msg.data !== undefined ? msg.data : null,
        type: 'AKPacket',
      };
      ...
      const fn = (reply, ref) => {
        ...
        clearListener();
        if (reply.data.error) {
          rej(reply.data.error);
        } else {
          res(reply);
        }
      };
      self.messageSystem().on('message/internal', fn);

      function clearListener() {
        self.messageSystem().removeListener('message/internal', fn);
      }
    });
  }
  ...
  sendError(error, timestamp = Date.now()) {
    return new Promise((res) => {
      ...
      const message = {
        timestamp,
        exitCode: process.exitCode || null, // undefined normally
        env: {
          os: process.platform,
          engine: 'node',
