...
const PresenceSystem = () => {
  ...
  const presenceSystem = {
    initialize(options) {
      ...
      const registryRef = rootRef.child('registry').child(get.safeString(cid));
      ...
      if (listenMode === 'normal') {
        rootRef.child('messages').orderByChild('timeReceived').startAt(Date.now()).on('child_added', (message) => {
          const msg = message.val();
          // Filer out own messages
          if (msg.uid === cid) return;
